...
impl ANN{
    ...
    pub fn evo_train_ai_chunked(&self, testdata: &Vec<(Array<f32>, Array<f32>)>, population: usize, offspring: usize, mutation_speed: f32, generations: u64, generational_callback: Box<Fn(u64, HostANN) -> ()>) -> ANN{

        let input_chunks = testdata.chunks(10).map(|io|  join_many(1, io.iter().map(|(inputs, _outputs)| inputs).collect::<Vec<&Array<f32>>>())).collect::<Vec<Array<f32>>>();
        let output_chunks = testdata.chunks(10).map(|io|  join_many(1, io.iter().map(|(_inputs, outputs)| outputs).collect::<Vec<&Array<f32>>>())).collect::<Vec<Array<f32>>>();
        let ios = input_chunks.iter().zip(output_chunks.iter()).collect::<Vec<(&Array<f32>, &Array<f32>)>>();
        let num_chunks = ios.len() as f32;


        println!("Input dimensions: {:?} x {}, Output dimensions: {:?} x {}", input_chunks[0].dims().get(),input_chunks.len(), output_chunks[0].dims().get(), output_chunks.len());

        let num_tests = testdata.len() as u64;
        println!("Creating zoo");
        ...
        for gen in 0 .. generations{
            ...
            now = Instant::now();
            let mut test_result = zoo.into_iter().chain(offspring).map(|spec| (ios.iter().fold(0f32, |score, (input, output)| score + (ANN::test_on_arrays(&spec, input, output, 10)) / num_chunks), spec)).collect::<Vec<(f32, ANN)>>();
